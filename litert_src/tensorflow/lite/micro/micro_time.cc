...
extern "C" {
    ...
    extern uint32_t CORETIMER_FrequencyGet ( void );
    extern uint32_t CORETIMER_CounterGet ( void );
...
uint32_t ticks_per_second() { return CORETIMER_FrequencyGet(); }
...
uint32_t GetCurrentTimeTicks() { return CORETIMER_CounterGet(); }
